...
view: streak_inventory_pdt {
  derived_table: {
    ...
    sql:
      ...
      "eventque_business"."user_win_streak_count" AS "user_win_streak_count",
      ...
      GROUP BY "user_win_streak_count", "event_date", "advertising_id", "install_dt", "country_code", "user_platform", "user_os_version","arrival_date"
      ;;


    publish_as_db_view: yes
    sql_trigger_value: SELECT DATE_TRUNC('day',DATEADD('minute', -480 , GETDATE() )  )  ;;
    sortkeys: ["advertising_id", "install_dt", "country_code", "user_platform", "user_win_streak_count", "event_date"]
  ...
  dimension: user_win_streak_count {
    type: number
    sql: case when ${TABLE}.user_win_streak_count is null then 0 else ${TABLE}.user_win_streak_count end ;;
  }
